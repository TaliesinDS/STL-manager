...
def translate_tokens(tokens: List[str], glos: Glossary, *, dedup: bool = True) -> List[str]:
    """Translate tokens using the glossary.

    Parameters
    ----------
    dedup : bool
        When *True* (default), deduplicate the output while preserving order.
        Pass *False* when building human display strings that may contain
        intentional repetition (e.g. "Knights and Knights").
    """
    out: List[str] = []
    i = 0
    n = len(tokens)
...
# UI display functions extracted to scripts/lib/ui_display.py
from scripts.lib.ui_display import build_ui_display  # noqa: E402, F401
