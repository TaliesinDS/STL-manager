...
SCRIPTS_DIR = THIS_FILE.parents[1]  # .../scripts
REPO_ROOT = THIS_FILE.parents[2]    # repo root
...
def main(argv: list[str]) -> int:
    ...
    if args.designers_file:
        ...
        # Support relocated vocab files under /vocab (post restructure)
        candidate_new = REPO_ROOT / 'vocab' / 'designers_tokenmap.md'
        if candidate_new.exists():
            dpath = candidate_new
        else:
            legacy = REPO_ROOT / 'designers_tokenmap.md'
            if legacy.exists():
    ...
    if args.tokenmap:
        ...
        # Auto-detect relocated tokenmap in vocab/ first, fallback to repo root
        candidate_new = REPO_ROOT / 'vocab' / 'tokenmap.md'
        if candidate_new.exists():
            tm_path = candidate_new
        else:
            legacy = REPO_ROOT / 'tokenmap.md'
            if legacy.exists():
    ...
    if args.ignore_file:
        ...
        # Fallback search order for ignored_tokens.txt
        # 1) Canonical staged location under scripts/30_normalize_match/
        # 2) vocab/ (if stored as general data at repo root)
        # 3) Alongside scripts/ (legacy)
        candidates: list[tuple[pathlib.Path, str]] = []
        candidates.append((SCRIPTS_DIR / '30_normalize_match' / 'ignored_tokens.txt', 'staged'))
        candidates.append((REPO_ROOT / 'vocab' / 'ignored_tokens.txt', 'vocab'))
        candidates.append((SCRIPTS_DIR / 'ignored_tokens.txt', 'legacy'))
        for path, label in candidates:
            if path.exists():
                ig_path = path
                pretty = f"{ig_path.name} ({label})"
                print(f"[info] --ignore-file not provided; using default {pretty}")
                break
    if ig_path:
        if ig_path.exists():
            try:
