"""Compatibility shim for scripts.check_character_conflicts.

Canonical module moved to scripts/60_reports_analysis/check_character_conflicts.py.
...
_IMPL_PATH = _ROOT / "scripts" / "60_reports_analysis" / "check_character_conflicts.py"

if str(_ROOT) not in sys.path:
    sys.path.insert(0, str(_ROOT))

if not _IMPL_PATH.is_file():
    raise FileNotFoundError(f"Relocated check_character_conflicts not found at {_IMPL_PATH}")

_SPEC = importlib.util.spec_from_file_location(
    "scripts.check_character_conflicts_impl", str(_IMPL_PATH)
)
