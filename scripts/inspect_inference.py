...
def _load_and_run(argv: list[str] | None = None) -> int:
    ...
    canonical = scripts_dir / '60_reports_analysis' / 'inspect_inference.py'

    proj_root = scripts_dir.parent
    if str(proj_root) not in sys.path:
        sys.path.insert(0, str(proj_root))

    spec = importlib.util.spec_from_file_location('scripts.60_reports_analysis.inspect_inference', canonical)
    if spec is None or spec.loader is None:
        raise RuntimeError(f'Cannot import canonical script at: {canonical}')
    mod = importlib.util.module_from_spec(spec)
    sys.modules['scripts.60_reports_analysis.inspect_inference'] = mod
